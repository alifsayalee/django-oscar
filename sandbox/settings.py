...
INSTALLED_APPS = [
    ...
    # Sandbox PayPal payments API (additive; reuses Oscar's order models)
...
# PayPal
# ======
# Read entirely from the environment at run time. Never hard-code the values.
# PAYPAL_BASE_URL is an optional verbatim override: when set it is used as the API base
# address for every PayPal call (including the OAuth token request); otherwise the base URL
# is derived from PAYPAL_ENVIRONMENT.
...
# The payments API logs (no card data, no secrets — the boundary redacts).
LOGGING['loggers']['payments_api'] = {
    'handlers': ['console'],
    'level': 'INFO',
    'propagate': False,
}
